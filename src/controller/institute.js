...
const addUpdateInstitute = async (req, res) => {
  try {
    ...
    if (_id) {
      ...
      return res.status(200).json({ success:true, message: 'institute updated successfully' });
    } else {

     const isInstitutePresent = await Institute.findOne({ $or: [
     ...
     restData.instituteId = `INS${Date.now()}`;
      ...
      return res.status(201).json({ success:true, message: 'institute added successfully' });
    }
  } catch (error) {
...
const getInstitute = async (req, res) => {
    try {
      const { data } = req.params;
        ...
        const institute = await Institute.findOne({$or: [{email: data}, {contact: data}]});
        if (!institute) {
          console.log(`institute not found with ${data}`);
          return res.status(404).json({ success:false, message: `institute not found with ${data}` });
        }
...
module.exports = { addUpdateInstitute, getInstitute, deleteInstitute };
